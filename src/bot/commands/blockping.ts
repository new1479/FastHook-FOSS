...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("blockping")
        .setDescription("Blocks pings from being sent to the proxy.")
    ...
    async execute(interaction: CommandInteraction) {
        ...
        if (hook.blockping === set) return interaction.editReply({ "content": `blockping is already set to ${set}` });

        await webhook.updateOne({ uuid: id }, { blockping: true });

        return interaction.editReply({
            "embeds": [
                {
                    "title": "blockping",
                    "description": `blockping has been set to ${set}`,
                    "color": 7572187,
                    "author": {
