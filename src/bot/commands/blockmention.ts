...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("blockmention")
        .setDescription("Blocks mentions from being sent to the proxy.")
    ...
    async execute(interaction: CommandInteraction) {
        ...
        if (hook.blockmention === set) return interaction.editReply({ "content": `blockmention is already set to ${set}` });

        await webhook.updateOne({ uuid: id }, { blockmention: true });

        return interaction.editReply({
            "embeds": [
                {
                    "title": "blockmention",
                    "description": `blockmention has been set to ${set}`,
                    "color": 7572187,
                    "author": {
